...
/* USER CODE BEGIN PV */
static volatile int16_t key = -1;
const uint16_t password[]={7,9,3,2,12};
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(HAL_GetTick()>=2000+wait && pos!=0)
	  {
		  pos=0;
		  printf(" Timeout:\n");
		  wait=HAL_GetTick();
	  ...
	  else if(key!=-1)
	  {
		  ...
		  if(key==password[pos])
		  {
			  printf("pozice %d prijata\n",pos);
			  pos++;
		  }
		  else
		  {
			  printf("pozice %d spatne\n",pos);
			  pos=0;
		  }
		  if(pos==5)
		  {
			  printf("Heslo prijato\n");
